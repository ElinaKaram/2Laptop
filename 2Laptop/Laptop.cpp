...
CPU::CPU(const CPU& other) : brand(other.brand)
{
...
SSD::SSD(const SSD& other) : brand(other.brand)
{
...
VideoCard::VideoCard(const VideoCard& other) : brand(other.brand)
{
    memory = new int(*other.memory);
}
...
RAM::RAM(const RAM& other) : brand(other.brand)
{
...
Laptop::Laptop(const Laptop& other)
    : name(other.name), price(other.price), color(other.color),
    cpu(other.cpu), ssd(other.ssd), videoCard(other.videoCard), ram(other.ram)
{
    laptopCount++;
}
